...
/**
 * Replace all connection addresses (session-level and per-media c= lines)
 * with the given publicAddress. Required so SDP advertises a routable IP
 * instead of a private/bind address.
 */
exports.setConnectionAddress = function(sdp, publicAddress) {
  if(sdp.c) {
    sdp.c = { nettype: sdp.c.nettype || 'IN', addrtype: sdp.c.addrtype || 'IP4', address: publicAddress };
  }
  if(sdp.o) {
    sdp.o.address = publicAddress;
  }
  if(sdp.m) {
    sdp.m.forEach(function(m) {
      if(m.c) {
        m.c = { nettype: m.c.nettype || 'IN', addrtype: m.c.addrtype || 'IP4', address: publicAddress };
      }
    });
  }
  return sdp;
