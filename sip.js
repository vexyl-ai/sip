...
function parseAOR(data) {
  var r = applyRegex(/((?:[^\s"<>]+(?:\s+[^\s"<>]+)*)|"[^"\\]*(?:\\.[^"\\]*)*")?\s*\<\s*([^>]*)\s*\>|((?:[^\s@"<]@)?[^\s;]+)/g, data);

  var name = r[1];
  if(name && name[0] === '"' && name[name.length-1] === '"') {
    name = name.substring(1, name.length - 1).replace(/\\(.)/g, '$1');
  }

  return parseParams(data, {name: name, uri: r[2] || r[3] || ''});
}
...
function parseVia(data) {
  var r = applyRegex(/SIP\s*\/\s*(\d+\.\d+)\s*\/\s*([\S]+)\s+([\w\-\.]+|\[[0-9A-Fa-f:\.]+\])(?:\s*:\s*(\d+))?/g, data);
...
var parsers = {
  ...
  'reason': function(data) {
    // RFC 3326: Reason = protocol *( ";" reason-param )
    // e.g. "SIP;cause=603;text="Decline"" or "Q.850;cause=21;text="Call Rejected""
    var s = data.s;
    var parts = s.match(/^\s*([^\s;]+)/);
    var reason = {protocol: parts ? parts[1] : s};
    var re = /;\s*([\w\-.]+)\s*=\s*(?:"([^"]*)"|([^;"\s]+))/g;
    var m;
    while((m = re.exec(s)) !== null) {
      reason[m[1].toLowerCase()] = m[2] !== undefined ? m[2] : m[3];
    }
    if(reason.cause) reason.cause = +reason.cause;
    return reason;
  }
};
...
function parseUri(s) {
  ...
  var re = /^(sips?):(?:([^\s>:@]+)(?::([^\s@>]+))?@)?((?:[\w\-\.]+)|(?:\[[0-9A-Fa-f:\.]+\]))(?::(\d+))?((?:;[^\s=\?>;]+(?:=[^\s?\;]+)?)*)(?:\?(([^\s&=>]+=[^\s&=>]+)(&[^\s&=>]+=[^\s&=>]+)*))?$/;
...
var stringifiers = {
  ...
  'reason': function(h) {
    var s = h.protocol || 'SIP';
    if(h.cause !== undefined) s += ';cause=' + h.cause;
    if(h.text) s += ';text="' + h.text + '"';
    return 'Reason: ' + s + '\r\n';
  }
};
...
function makeResponse(rq, status, reason, extension) {
  var to = {uri: rq.headers.to.uri, params: Object.assign({}, rq.headers.to.params)};
  if(rq.headers.to.name) to.name = rq.headers.to.name;

  // RFC 3261 §8.2.6.2 — final responses (>=200) MUST have a To-tag
  if(status >= 200 && !to.params.tag) {
    to.params.tag = generateTag();
  }
  ...
  // RFC 3261 §8.2.6 — Contact header in 1xx and 2xx responses to INVITE
  if(rq.headers.contact && rq.method === 'INVITE' && status >= 100 && status < 300) {
    rs.headers.contact = rq.headers.contact;
  }

  // RFC 3261 §20.35 — Server header SHOULD be present in responses
  rs.headers.server = 'Vexyl-SIP/1.0';
...
// Convenience: create 6xx global failure responses with proper Reason header (RFC 3326)
// Usage: sip.makeDeclineResponse(rq)             → 603 Decline
//        sip.makeUnwantedResponse(rq, 'spam')     → 607 Unwanted
//        sip.makeRejectedResponse(rq, 'blocked')  → 608 Rejected
exports.makeDeclineResponse = function(rq, text) {
  return makeResponse(rq, 603, 'Decline', {
    headers: { reason: {protocol: 'SIP', cause: 603, text: text || 'Decline'} }
  });
};

exports.makeUnwantedResponse = function(rq, text) {
  return makeResponse(rq, 607, 'Unwanted', {
    headers: { reason: {protocol: 'SIP', cause: 607, text: text || 'Unwanted'} }
  });
};

exports.makeRejectedResponse = function(rq, text) {
  return makeResponse(rq, 608, 'Rejected', {
    headers: { reason: {protocol: 'SIP', cause: 608, text: text || 'Rejected'} }
  });
};
...
function makeStreamParser(onMessage, onFlood, maxBytesHeaders, maxContentLength) {
  ...
  function headers(data) {
    ...
    // HAProxy PROXY protocol v1 support
    var pp = r.match(/^PROXY (\S+) (\S+) (\S+) (\S+) (\S+)\r\n([\S\s]*)$/);
    if(pp) {
      p = {protocol: pp[1], sourceAddress: pp[2], destinationAddress: pp[3], sourcePort: +pp[4], destinationPort: +pp[5]};
      r = pp[6];
    }
...
function checkMessage(msg) {
  return (msg.method || (msg.status >= 100 && msg.status <= 999)) &&
    ...
    msg.headers.via[0].params &&
    msg.headers['call-id'] &&
    msg.headers.to &&
    msg.headers.from &&
    msg.headers.cseq &&
    typeof msg.headers.cseq === 'object' &&
    msg.headers.cseq.method &&
    typeof msg.headers.cseq.seq === 'number';
}

function makeStreamTransport(protocol, maxBytesHeaders, maxContentLength, connect, createServer, callback, errorLog) {
  ...
  function init(stream, remote) {
    ...
    var onMessage= function(m, proxy) {

      if(checkMessage(m)) {
        if(m.method) m.headers.via[0].params.received = remote.address;
        var remoteInfo = {protocol: remote.protocol, address: stream.remoteAddress, port: stream.remotePort, local: { address: stream.localAddress, port: stream.localPort}};
        if(proxy) remoteInfo.proxy = proxy;
        callback(m, remoteInfo, stream);
      }
    ...
    var onFlood= function() {
      ...
      if(errorLog) errorLog("Flood attempt, destroying stream");
...
function makeTlsTransport(options, callback) {
  var errorHandler = (options.logger && options.logger.error) || function() {};
...
function makeWsTransport(options, callback) {
  ...
  function makeClient(uri) {
    ...
    function send_open(m) { socket.send(Buffer.from(typeof m === 'string' ? m : stringify(m), 'binary')); }
  ...
  function get(flow) {
    ...
    if(ws) {
      return {
        ...
        var wsErrorLog = (options && options.logger && options.logger.error) || function() {};
        wsErrorLog("Failed to get ws for target. Target/flow: " + util.inspect(flow) + " Flows: " + util.inspect(flows));
    }
  }
...
function makeUdpTransport(options, callback) {
  function onMessage(data, rinfo) {
    ...
    if(msg && checkMessage(msg)) {
      ...
      // RFC 3261 §18.2.2 / RFC 3581: determine response destination
      // 1. If rport is set (non-null), use it (symmetric NAT)
      // 2. Else use Via sent-by port
      // 3. Else use source port from rinfo
      var via = msg.headers.via[0];
      var responsePort = rinfo.port;
      if(via.params.rport && via.params.rport !== null) {
        responsePort = +via.params.rport;
      } else if(via.port) {
        responsePort = via.port;
      }

      // Response address: use received param if set, else Via sent-by host
      var responseAddress = via.params.received || rinfo.address;

      callback(msg, {protocol: 'UDP', address: responseAddress, port: responsePort, local: {address: address, port: port}});
    }
  }
  ...
  function wrap(obj, target) {
    return Object.create(obj, {send: {value: function(m) {
      if(m.method) {
        ...
        m.headers.via[0].host = net.isIPv6(host) ? '[' + host + ']' : host;
        ...
        // RFC 3261 §8.1.1.6 — Max-Forwards MUST be present in all requests
        if(m.headers['max-forwards'] === undefined) {
          m.headers['max-forwards'] = 70;
        }

        // RFC 3261 §20.35 — User-Agent SHOULD be present
        if(!m.headers['user-agent']) {
          m.headers['user-agent'] = 'Vexyl-SIP/1.0';
        }
      }
...
function resolve(uri, action) {
  if(uri.params.transport === 'ws')
    return action([{protocol: uri.schema === 'sips' ? 'WSS' : 'WS', host: uri.host, port: uri.port || (uri.schema === 'sips' ? 443 : 80)}]);
  ...
  if(uri.port) {
    ...
    // Helper: resolve SRV records for given protocols (existing behavior)
    function resolveSrvForProtocols(protocols, host, cb) {
      ...
      protocols.forEach(function(proto) {
        resolveSrv('_sip._'+proto+'.'+host, function(e, r) {
          ...
          if(Array.isArray(r)) {
            // RFC 2782 — sort SRV by priority (ascending), then by weight
            r.sort(function(a, b) { return (a.priority - b.priority) || (b.weight - a.weight); });
            n += r.length;
    ...
    // Helper: resolve addresses from NAPTR records per RFC 3263
    function resolveFromNaptr(naptrRecords, cb) {
      ...
      function resolveSrvTarget(naptr, proto) {
          ...
          if(Array.isArray(srvRecords) && srvRecords.length > 0) {
            ...
            srvRecords.forEach(function(srv) {
              resolve46(srv.name, function(e, addrs) {
                ...
                if(--srvPending === 0 && --pending === 0) finalize();
              });
            });
          } else {
      ...
      sipRecords.forEach(function(naptr) {
        ...
        if(flags === 'a') {
          ...
          // 's' flag or default: replacement is an SRV target
          resolveSrvTarget(naptr, proto);
        }
      });
    }
...
function createInviteClientTransaction(rq, transport, tu, cleanup, options) {
  ...
  var timerA = options.timerA || 500;   // initial retransmit interval
  var timerB = options.timerB || 32000; // transaction timeout
  ...
  return {
    ...
    shutdown: function() { sm.enter(terminated); },
    // #155: Allow caller to force a shorter timeout after CANCEL (default 32s)
    cancelSent: function() {
      if(ringTimeLimit) {
        clearTimeout(ringTimeLimit);
        ringTimeLimit = setTimeout(function() {
          tu(makeResponse(rq, 487, 'Request Terminated'));
          sm.enter(terminated);
        }, options.cancelTimeout || 32000);
      }
    }
  };
}

function createClientTransaction(rq, transport, tu, cleanup, options) {
  options = options || {};
...
function makeTransactionLayer(options, transport) {
  ...
  return {
    createServerTransaction: function(rq, cn) {
      ...
      send.reliable = cn.protocol && cn.protocol.toUpperCase() !== 'UDP';
...
// RFC 3261 §21.6: 6xx responses are global failures — must not retry on any other address.
// 603 Decline: callee explicitly declined
// 607 Unwanted (RFC 8197): call flagged as spam/unwanted
// 608 Rejected: call rejected by intermediary
function isGlobalFailure(status) {
  return status >= 600 && status <= 699;
}

exports.isGlobalFailure = isGlobalFailure;
...
function sequentialSearch(transaction, connect, addresses, rq, callback) {
  ...
  function searching(rs) {
    ...
    // 6xx Global Failure: stop all retries immediately (RFC 3261 §21.6)
    if(isGlobalFailure(rs.status)) {
      addresses.length = 0; // clear remaining addresses — no retry allowed
      onresponse = callback;
      callback(rs);
      return;
    }

    // RFC 3263 §4.3 — failover on transport errors and 503
    if(rs.status === 503 || rs.status === 408)
      return next();
    ...
    send: function(m, callback) {
      if(m.method === undefined) {
        ...
        if(m.headers.route && m.headers.route.length > 0) {
          var routeHop = parseUri(m.headers.route[0].uri);
          if(routeHop) {
            hop = routeHop;
