...
function generateNonce(tag, timestamp) {
  ...
  return Buffer.from([ts, kd(ts, tag, nonceSalt)].join(';'), 'ascii').toString('base64');
}
exports.generateNonce = generateNonce;

function extractNonceTimestamp(nonce, tag) {
  var v = Buffer.from(nonce, 'base64').toString('ascii').split(';');
  if(v.length != 2)
    return;
...
function initClientContext(ctx, rs, creds) {
  ...
  // RFC 2617 §3.2.1 — stale=true means nonce expired but credentials are still valid
  var isStale = challenge.stale && unq(lowercase(challenge.stale)) === 'true';
  ctx.stale = isStale;

  if(ctx.nonce !== unq(challenge.nonce) || isStale) {
    ...
    // RFC 2617 §3.2.1 — domain defines protection scope
...
exports.authenticateResponse = function(ctx, rs) {
  ...
  var digest=calculateDigest({ha1:ctx.ha1, method:'', nonce:ctx.nonce, nc:numberTo8Hex(ctx.nc), cnonce:ctx.cnonce, qop:ctx.qop, uri:ctx.uri, entity:rs.content});
